					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('plantilla/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('plantilla/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
								...
								<table class="table table-striped hover">
									<thead>
										<tr>
											<?php 
												$headers = array(
												    array('column_header'=>'Position No','column_field'=>'positionCode','width'=>'w-5','align'=>''),
												    array('column_header'=>'Job Title','column_field'=>'jobTitleID','width'=>'w-10','align'=>''),
												    array('column_header'=>'Branch','column_field'=>'branchID','width'=>'w-10','align'=>''),
												    array('column_header'=>'Department','column_field'=>'deptID','width'=>'w-15','align'=>''),
												    array('column_header'=>'Section','column_field'=>'divisionID','width'=>'w-15','align'=>''),
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<input type="text" class="form-control" id="positionCode" name="positionCode" value="<?php echo $positionCode ?>">
											</th>
											<th class="form-group form-input">
												<?php 
													$this->db->where('status',1);
													$jts = $this->db->get('job_titles')->result();
													?>
												<select class="form-control" id="jobTitleID" name="jobTitleID"  >
													<option value="">&nbsp;</option>
													<?php foreach($jts as $jt) {?>
													<option value="<?php echo $jt->jobTitleID ?>" <?php if ($jt->jobTitleID == $jobTitleID) echo "selected"; ?>><?php echo $jt->code.' - '.$jt->jobTitle  ?></option>
													<?php } ?>
												</select>
											</th>
											<th class="form-group form-input">
											...
											<th class="form-group form-input">
												<select id="deptID" name="deptID" class="form-control">
													...
													<?php 
														...
														foreach($results as $res){
														?>
													<option value="<?php echo $res->deptID?>" <?php if ($res->deptID == $deptID) { echo "selected"; }?> onchange="get_divisions();"><?php echo $res->deptCode?></option>
													<?php }?>
												</select>
											</th>
											<th class="form-group form-input">
											...
											<th>
												<select class="form-control" id="status" name="status" style="width:100px">
													<option value="">&nbsp;</option>
													<option value="1" <?php if($status == "1") echo "selected"; ?>>Vacant</option>
													<option value="2" <?php if($status == "2") echo "selected"; ?>>Occupied</option>
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
										<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->jobPositionID); ?>'">
											<td><?php echo $row->positionCode  ?></td>
