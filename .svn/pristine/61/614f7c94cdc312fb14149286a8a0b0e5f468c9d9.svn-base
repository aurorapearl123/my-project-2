...
class Family_member extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->module       = 'Family Member';
        $this->table        = 'employee_family_members';                                                 
        $this->pfield       = $this->data['pfield'] = 'familyID';                                                 
        $this->logfield     = 'name';
        $this->module_path  = 'modules/employee/employee';           
        $this->data['controller_page']  = $this->controller_page = site_url('family_member');
    ...
    public function save()
    {
        ...
        $table_fields = array('name', 'relationship', 'civilStatus', 'occupation', 'address', 'isDependent', 'remarks');
        ...
        if ($this->userrole_model->has_access($this->session->userdata('current_userID'), 'Add '.$this->module)) {
            ...
            if (trim($this->input->post('birthDate')) != '') {
                $this->db->set('birthDate', date('Y-m-d', strtotime(trim($this->input->post('birthDate')))));
            }
    ...
    public function update()
    {
        ...
        if ($this->userrole_model->has_access($this->session->userdata('current_userID'), 'Edit Existing '.$this->module)) {
            ...
            if (trim($this->input->post('birthDate')) != '') {
                $this->records->fields['birthDate'] = date('Y-m-d', strtotime(trim($this->input->post('birthDate'))));
            }
    ...
    public function delete($id=0)
    {
        ...
        $id = trim($this->input->post('familyID'));
    ...
    public function show()
    {
        ...
        if ($this->userrole_model->has_access($this->session->userdata('current_userID'), 'View '.$this->module)) {
            ...
            $this->db->select('employee_family_members.*');
            $this->db->select('DATE_FORMAT(employee_family_members.birthDate, "%m/%d/%Y") as birthDate');
            $this->db->where('employee_family_members.empID', $empID);
            $records = $this->db->get('employee_family_members')->result();
