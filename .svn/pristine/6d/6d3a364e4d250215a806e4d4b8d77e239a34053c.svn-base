...
class Order extends CI_Controller {
    ...
    public function save() {
        ...
        if ($this->roles ['create']) {
            ...
            $osSeries= $this->db->get('seriesno')->row();
            
            $osSeriesNo = $osSeries->osNo + 1;
            
            require_once(APPPATH.'controllers/Generic_ajax.php');
            $seriesNo = Generic_ajax::updateSeriesNo('osNo',$osSeriesNo,$userCurrentBranch);

            $this->records->fields['osNo']           =   str_pad($osSeriesNo,4,'0',STR_PAD_LEFT);        
            ...
            if ($this->records->save ()) {
                ...
                $services_data = $this->input->post('services_data');
...
//            $this->db->select('order_details.serviceID');
            ...
            $this->db->select('clothes_categories.clothesCatID');
            $this->db->from('order_details');
            $this->db->join('order_detail_categories', 'order_details.id=order_detail_categories.order_detail_id', 'right');
            $this->db->join('clothes_categories', 'clothes_categories.clothesCatID=order_detail_categories.clothes_catID', 'right');
...
//            die();
    ...
    public function updateComments() {
        ...
        $this->submenu ();
        $data = $this->data;
        $table_fields = array ('adminComment');
...
//            die();
    ...
    public function show() {
        ...
        // // sorting fields
        $sorting_fields = array ('osNo' => 'desc' );
    ...
    public function insert_order_details($orderID, $services_data, $categories_data)
    {

        $order_details = json_decode($services_data, true);
        $categories_data = json_decode($categories_data, true);
        $id = 0;
        foreach($order_details as $order_detail) {
            $id = $this->insertDetails($categories_data, $orderID, $order_detail['quantity'], $order_detail['amount'], $order_detail['rate'], $order_detail['unit'], $order_detail['service_id']);
        }
        return $id;
 ...
 
    {
        ...
        $approvedBy = Generic_ajax::updateCancelledBy($this->table, "orderID",  $orderID, $this->session->userdata('current_user')->userID);
    ...
    public function updateColumn($table, $value, $column, $id)
    {
        $data = [
            $column => $value,
        ];
    ...
    public function getOrdersDetailsById($orderID)
    {
        ...
        foreach($orders as $order) {
            $data[] = [
                ...
                'user_middle_name' => $order->middleName,
                ...
                'dateReleased' => $order->dateReleased,
                'dateCancelled' => $order->dateCancelled,
                'dateWashed' => $order->dateWashed,
                'dateDry' => $order->dateDry,
                'dateFold' => $order->dateFold,
                'osNo' => $order->osNo,
