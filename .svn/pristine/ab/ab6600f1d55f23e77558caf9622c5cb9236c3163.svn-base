...
class Jobtitle extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data ['controller_page'] = $this->controller_page = site_url ( 'jobtitle' ); // defines contoller link
		$this->table = 'job_titles'; // defines the default table
		$this->pfield = $this->data ['pfield'] = 'jobTitleID'; // defines primary key
		$this->logfield = 'companyCode';
		$this->module_path = 'modules/' . strtolower ( str_replace ( " ", "_", $this->module ) ) . '/jobtitle'; // defines module path
	...
	private function submenu() {
		...
		$this->data ['current_module'] = $this->modules [$this->module] ['sub'] ['Jobtitle']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	private function _in_used($id = 0) {
		$tables = array ('job_positions' => 'jobTitleID' );
	...
	public function save() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('code', 'jobTitle', 'jobDescription' );
		...
		if ($this->roles ['create']) {
			...
			if ($this->records->save ()) {
				$this->records->fields = array ();
				$id = $this->records->where ['jobTitleID'] = $this->db->insert_id ();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('code', 'jobTitle', 'jobDescription', 'status' );
	...
	public function show() {
		...
		$condition_fields = array (array ('variable' => 'code', 'field' => $this->table . '.code', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'jobTitle', 'field' => $this->table . '.jobTitle', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'jobDescription', 'field' => $this->table . '.jobDescription', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('code' => 'asc' );
	...
	public function printlist() {
		...
		// variable:field:default_value:operator
		// note: dont include the special query field filter
		$condition_fields = array (array ('variable' => 'code', 'field' => $this->table . '.code', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'jobTitle', 'field' => $this->table . '.jobTitle', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'jobDescription', 'field' => $this->table . '.jobDescription', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'remarks', 'field' => $this->table . '.remarks', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'rank', 'field' => $this->table . '.rank', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
	...
	function exportlist() {
		...
		$data = "<?xml version='1.0'?>
    		...
    		<Table>
    		<Column ss:Index='1' ss:AutoFitWidth=\"1\" ss:Width='50.00'/>
    		<Column ss:Index='2' ss:AutoFitWidth=\"1\" ss:Width='150.00'/>
    		<Column ss:Index='3' ss:AutoFitWidth=\"1\" ss:Width='250.00'/>
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->jobDescription . "</Data></Cell>";
				if ($row->status == 1) {
