...
class Salary_table extends CI_Controller
{
    //Default Variables
    var $menu;
    var $roles;
    ...
    public function __construct()
    {
        ...
        $this->table        = 'schedule_salary';                                                 
        $this->pfield       = $this->data['pfield'] = 'id';                                                 
        $this->logfield     = 'grade';
        $this->module_path  = 'modules/salary_and_wages/salary_table';           
        $this->data['controller_page']  = $this->controller_page = site_url('salary_table');
    ...
    private function submenu()
    {
        ...
        $this->data['current_module']   = $this->modules[$this->module]['sub']['Salary Table'];      // defines the current sub module
        // check roles
        $this->check_roles();
    ...
    public function save()
    {
        ...
        if ($this->roles['create']) {
            $batch = array();
            $gradeLimit 	= $this->config_model->getConfig('Salary Grade Limit');
            $stepLimit 		= $this->config_model->getConfig('Salary Step Limit');
            	
            if (count($this->input->post('employeeTypeID'))) {
                foreach ($this->input->post('employeeTypeID') as $employeeTypeID)	 {
                    for ($grade = 1; $grade <= $gradeLimit; $grade++) {
                        for ($step = 1; $step <= $stepLimit; $step++) {
                            $info = array();
                            $info['grade'] 	 			= $grade;
                            $info['step'] 				= $step;
                            $info['salary'] 			= $this->input->post('salary_'.$grade.'_'.$step);
                            $info['employeeTypeID'] 	= $employeeTypeID;
                    ...
                    $this->log_model->table_logs($this->module, $this->table, $this->pfield, $employeeTypeID, 'Insert', $logs);
                }
            }
            	
            $this->db->insert_batch('schedule_salary', $batch);
    ...
    public function edit($id)
    {
        ...
        if ($this->roles['edit']) {
			...
			$data['rec'] = $this->db->get('employee_types', 1)->row();
			...
			$this->db->where($this->table.'.employeeTypeID', $id);
			...
			if ($schedules->num_rows()) {
				foreach ($schedules->result() as $row) {
					$data['schedule'][$row->grade][$row->step] = $row->salary;
				}
			}
    ...
    public function update()
    {
        ...
        if ($this->roles['edit']) {
            ...
            $this->db->set('salary', $amount);
            $this->db->where('employeeTypeID', $employeeTypeID);
            $this->db->where('grade', $grade);
            $this->db->where('step', $step);
            $this->db->update('schedule_salary');
            
            $logs = "Record - Grade ".$grade." Step ".$step;
            $this->log_model->table_logs($this->data['current_module']['module_label'], $this->table, 'employeeTypeID', $employeeTypeID, 'Update', $logs);
    ...
    public function show()
    {
    	...
    	// join   	
    	$this->db->join('employee_types', $this->table.'.employeeTypeID=employee_types.employeeTypeID', 'left');
        ...
        $this->db->group_by($this->table.'.employeeTypeID');
