					...
					<div class="card-head">
						...
						<div class="card-head-tools">
							<ul class="tools-list">
								...
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Print" onclick="popUp('<?php echo site_url('service_type/printlist') ?>', 800, 500)"><i class="la la-print"></i></button>
								</li>
								<li>
									<button type="button" class="btn btn-outline-light bmd-btn-icon" data-toggle="tooltip" data-placement="bottom" title="Export to Excel File" onclick="window.location='<?php echo site_url('service_type/exportlist') ?>';"><i class="la la-file-excel-o"></i></button>
								</li>
							</ul>
						</div>
					</div>
								...
								<table class="table table-striped has-img hover">
									<thead>
										<tr>
											<?php 
												$headers = array(												    
												    array('column_header'=>'Service Type','column_field'=>'serviceType','width'=>'w-25','align'=>''),
												    array('column_header'=>'Regular Rate','column_field'=>'regRate','width'=>'w-25','align'=>''),
												    array('column_header'=>'Discounted Rate','column_field'=>'discountedRate','width'=>'w-25','align'=>''),
										...
										<tr id="filter-group" class="collapse multi-collapse table-filter show">
											<th class="form-group form-input">
												<input type="text" class="form-control" id="serviceType" name="serviceType" style="width:250px" value="<?php echo $serviceType ?>">
											</th>

											<th class="form-group form-input">
												<input type="text" class="form-control" id="regRate" name="regRate" style="width:250px" value="<?php echo $regRate ?>">
											</th>
											
											<th class="form-group form-input">
												<input type="text" class="form-control" id="discountedRate" name="discountedRate" style="width:250px" value="<?php echo $discountedRate ?>">
											</th>
											<th>
									...
									<tbody>
										<?php 
											if (count($records)) {
											    ...
											    ?>
										<tr onclick="location.href='<?php echo $controller_page."/view/".$this->encrypter->encode($row->serviceID); ?>'">
											<td><?php echo $row->serviceType ?></td>
											<td><?php echo $row->regRate ?></td>
											<td><?php echo $row->discountedRate ?></td>
											<td align="center">
												<?php 
													if ($row->status == 1) {
