...
class Education_background extends CI_Controller
{
    //Default Variables
    var $common_menu;
    var $roles;
    ...
    public function __construct()
    {
        ...
        // set variables
        $this->data['current_module'] = $this->module = 'Education Background';
        $this->module_label = 'Education Backgrounds';
        $this->table        = 'employee_education_backgrounds';
        $this->module_path  = 'modules/Employee/Education_Background';
        $this->module_path_submenu  = 'modules/Employee/submenu';
        $this->pfield = 'educationID';
        $this->logfield = 'level';
        $this->data['controller_page'] = $this->controller_page = site_url('education_background');
    ...
    public function create()
    {
        ...
        if ($this->roles['create']) {
            $data['required_fields'] = array('empID'=>'Employee','level'=>'Level of Education', 'degree'=>'Degree','school'=>'School',
                'yearStart'=>'Year Start','status'=>'Status');
    ...
    public function save()
    {
        ...
        if ($this->roles['create']) {
            ...
            $table_fields = array('empID','level', 'degree', 'school', 'yearStart','yearGrad','levelEarn','honors','remarks','status');
            ...
            if ($this->record->save()) {
                ...
                $this->record->where['level']= trim($this->input->post('level'));
                $this->record->where['degree']= trim($this->input->post('degree'));
                $this->record->where['school']= trim($this->input->post('school'));
                $this->record->where['yearStart']= trim($this->input->post('yearStart'));
                ...
                $data['urlredirect']    = $this->controller_page."/view/".$this->encrypter->encode($recordID);
                $this->load->view('header', $data);
                $this->load->view('message');
                $this->load->view('footer');
    ...
    public function get_education_backgrounds()
    {
        $this->db->order_by('name','asc');
        $records = $this->db->get('education_backgrounds');
        echo $this->frameworkhelper->get_json_data($records, 'educationID', array('level'=>' - ','degree'=>''));
    }
