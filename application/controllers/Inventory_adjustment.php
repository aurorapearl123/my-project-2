...
class Inventory_adjustment extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data['controller_page']  = $this->controller_page = site_url('inventory_adjustment');// defines contoller link
        $this->table        = 'inventory_adjustments';
        $this->pfield = $this->data['pfield'] = 'adjID'; // defines primary key
        // $this->logfield     = 'deptCode';
        $this->module_path  = 'modules/'.strtolower(str_replace(" ","_",$this->module)).'/inventory_adjustment';// defines module path / folder name
    ...
    public function save()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('date','branchID','itemID','adjType','qty','remarks','createdBy');
        ...
        if ($this->roles['create']) {
            ...
            $adjSeries= $this->db->get('seriesno')->row();
            
            $adjSeriesNo = $adjSeries->adjNo + 1;
    ...
    public function update()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('branchID','itemID','adjType','qty','remarks','createdBy','status');
    ...
    public function view($id)
    {
        ...
        if ($this->roles['view']) {
            ...
            // extend fields - join tables
            $this->record->fields[] = $this->table.'.dateCreated';
            $this->record->fields[] = $this->table.'.dateApproved';
            $this->record->fields[] = $this->table.'.dateCancelled';
            ...
            $this->record->fields[] = 'approvedByUser.firstName as approvedByFirst';
            $this->record->fields[] = 'approvedByUser.middleName as approvedByMiddle';
            $this->record->fields[] = 'approvedByUser.lastName as approvedByLast';
            ...
            $this->record->joins[]  = array('users as approvedByUser',$this->table.'.approvedBy=approvedByUser.userID','left');
    ...
    function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->item."</Data></Cell>";
                ...
                if  ($row->adjType == 'DR') {
                    $data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Debit</Data></Cell>";
                } else {
                    $data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Credit</Data></Cell>";
                }
    ...
    public function check_duplicate()
    {
        ...
        $this->db->where('itemID', trim($this->input->post('itemID')));
        $this->db->where('adjType', trim($this->input->post('adjType')));
    ...
    public function updateQTY()
    {
        ...
        $adjType = trim($this->input->post('adjType'));  
        ...
        $itemInventory = $this->db->get('item_inventory')->row();

        $this->db->where('branchID', $branchID);
        $seriesno = $this->db->get('seriesno')->row();

        //beginning balance
        $begBalance = $itemInventory->qty;

        if ($adjType == 'DR') {
            //adjustment type = 'debit' || 'add'
            $totalQty = $itemInventory->qty + $qty;
            $item_qty = Generic_ajax::updateQTY('item_inventory', $branchID, $itemID,$totalQty);
            //$branchID, $itemID, $refNo, $begbal,$debit,$credit,$endBal
            $item_stockCard = Generic_ajax::insertStockCard($branchID, $itemID, 'adjno'.str_pad($seriesno->adjNo,4,'0',STR_PAD_LEFT),$itemInventory->qty,$qty,0,$itemInventory->qty+$qty);
             
        } else {
            //adjustment type = 'credit' || 'subtract'
            $totalQty = $itemInventory->qty - $qty;
            $item_qty = Generic_ajax::updateQTY('item_inventory', $branchID, $itemID,$totalQty);
            //$branchID, $itemID, $refNo, $begbal,$debit,$credit,$endBal
            $item_stockCard = Generic_ajax::insertStockCard($branchID, $itemID, 'adjno'.str_pad($seriesno->adjNo,4,'0',STR_PAD_LEFT),$itemInventory->qty,0,$qty,$itemInventory->qty-$qty);
        }
