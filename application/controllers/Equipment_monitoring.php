...
class Equipment_monitoring extends CI_Controller
{
    //Default Variables
    var $menu;
    var $roles;
    ...
    public function __construct()
    {
        ...
        $this->data['controller_page']  = $this->controller_page = site_url('equipment_monitoring');// defines contoller link
        $this->table        = 'equipment_monitoring';        
        $this->pfield = $this->data['pfield'] = 'emID'; // defines primary key
        // $this->logfield     = 'deptCode';
        $this->module_path  = 'modules/'.strtolower(str_replace(" ","_",$this->module)).'/equipment_monitoring';// defines module path / folder name
    ...
    private function submenu()
    {
        ...
        $this->data['current_module']        = $this->modules[$this->module]['sub']['Equipment Monitoring'];      // defines the current sub module
        // check roles
        $this->check_roles();
    ...
    public function save()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('branchID','equipmentID','monitoredBy','remarks');
        ...
        if ($this->roles['create']) {
            ...
            $emAssetSeries= $this->db->get('seriesno')->row();

            $assetSeriesNo = $emAssetSeries->assetNo + 1;

            $data = array(
                'branchID'      =>  $userCurrentBranch,
                'equipmentID'   =>  $equipment_id,
                'assetNo'       =>  $assetSeriesNo

            );

            $this->db->insert('equipment_inventory', $data);

            
            $emSeriesNo = $emAssetSeries->emNo + 1;            
                       
            require_once(APPPATH.'controllers/Generic_ajax.php');
            $seriesNo = Generic_ajax::updateSeriesNo('emNo',$emSeriesNo,$userCurrentBranch);
            $seriesNo = Generic_ajax::updateSeriesNo('assetNo',$assetSeriesNo,$userCurrentBranch);

            $this->record->fields['emNo']           =   str_pad($emSeriesNo,4,'0',STR_PAD_LEFT);
            ...
            if ($this->record->save()) {
                $this->record->fields = array();
                $emID = $this->record->where['emID'] = $this->db->insert_id();
                ...
                $data["urlredirect"] = $this->controller_page."/view/".$this->encrypter->encode($emID);
                $this->load->view("header",$data);
                $this->load->view("message");
                $this->load->view("footer");
    ...
    public function update()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('branchID','equipmentID','remarks');
        ...
        if ($this->roles['edit']) {
            ...
            $this->record->fields['date']           =   date('Y-m-d');
            $this->record->fields['monitoredBy']    =   $this->session->userdata('current_user')->firstName.' '.$this->session->userdata('current_user')->middleName.' '.$this->session->userdata('current_user')->lastName;
    ...
    public function view($id)
    {
        ...
        if ($this->roles['view']) {
            ...
            $this->record->fields[] = 'createdByUser.firstName as createdByFirst';
            $this->record->fields[] = 'createdByUser.middleName as createdByMiddle';
            $this->record->fields[] = 'createdByUser.lastName as createdByLast';

            $this->record->fields[] = 'confirmedByUser.firstName as confirmedByFirst';
            $this->record->fields[] = 'confirmedByUser.middleName as confirmedByMiddle';
            $this->record->fields[] = 'confirmedByUser.lastName as confirmedByLast';

            $this->record->fields[] = 'cancelledByUser.firstName as cancelledByFirst';
            $this->record->fields[] = 'cancelledByUser.middleName as cancelledByMiddle';
            $this->record->fields[] = 'cancelledByUser.lastName as cancelledByLast';

            // set joins
            $this->record->joins[]  = array('branches',$this->table.'.branchID=branches.branchID','left');                        
            $this->record->joins[]  = array('equipments',$this->table.'.equipmentID=equipments.equipmentID','left');
            $this->record->joins[]  = array('users as createdByUser',$this->table.'.createdBy=createdByUser.userID','left');
            $this->record->joins[]  = array('users as confirmedByUser',$this->table.'.approvedBy=confirmedByUser.userID','left');
            $this->record->joins[]  = array('users as cancelledByUser',$this->table.'.cancelledBy=cancelledByUser.userID','left');
    ...
    public function show() {
        ...
        $condition_fields = array (
            array ('variable' => 'emNo', 'field' => $this->table . '.emNo', 'default_value' => '', 'operator' => 'like_both' ), 
            // array ('variable' => 'date', 'field' => $this->table . '.date', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'brand', 'field' => 'equipments.brand', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'model', 'field' => 'equipments.model', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'serialNo', 'field' => 'equipments.serialNo', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'name', 'field' => 'equipments.name', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'description', 'field' => 'equipments.description', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'description', 'field' => 'equipments.description', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'monitoredBy', 'field' => $this->table . '.monitoredBy', 'default_value' => '', 'operator' => 'like_both' ), 
        ...
        // // sorting fields
        $sorting_fields = array ('emNo' => 'desc' );
    ...
    public function printlist()
    {
        ...
        // note: dont include the special query field filter
        $condition_fields = array (
            array ('variable' => 'emNo', 'field' => $this->table . '.emNo', 'default_value' => '', 'operator' => 'where' ), 
            array ('variable' => 'date', 'field' => $this->table . '.date', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'brand', 'field' => 'equipments.brand', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'model', 'field' => 'equipments.model', 'default_value' => '', 'operator' => 'like_both' ), 
            array ('variable' => 'serialNo', 'field' => 'equipments.serialNo', 'default_value' => '', 'operator' => 'like_both' ), 
        ...
        $data['title'] = "Equipment Monitoring";
    ...
    function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->monitoredBy."</Data></Cell>";
        ...
        //Final XML Blurb
        $filename = "equipmenmt_monitoring";
    ...
    public function check_duplicate()
    {
        ...
        $this->db->where('equipmentID', trim($this->input->post('equipmentID')));      
        $this->db->where('monitoredBy', trim($this->input->post('monitoredBy')));      
    ...
    {
        $emID = trim($this->input->post('emID'));   
        require_once(APPPATH.'controllers/Generic_ajax.php');
        //update approved by        
        $approvedBy = Generic_ajax::updateApprovedBy($this->table, "emID", $emID, $this->session->userdata('current_user')->userID);
        $response = [
            'data' => [
                'status' => 200,                
    ...
    public function updateCancelledBy()
    {
        ...
        $approvedBy = Generic_ajax::updateCancelledBy($this->table, "emID", $emID, $this->session->userdata('current_user')->userID);
