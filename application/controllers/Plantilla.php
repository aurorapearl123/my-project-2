...
class Plantilla extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data ['controller_page'] = $this->controller_page = site_url ( 'plantilla' ); // defines contoller link
		$this->table = 'job_positions'; // defines the default table
		$this->pfield = $this->data ['pfield'] = 'jobPositionID'; // defines primary key
		$this->logfield = 'positionCode';
		$this->module_path = 'modules/' . strtolower ( str_replace ( " ", "_", $this->module ) ) . '/plantilla'; // defines module path
	...
	private function submenu() {
		...
		$this->data ['current_module'] = $this->modules [$this->module] ['sub'] ['Plantilla']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	private function _in_used($id = 0) {
		$tables = array ('departments' => 'branchID', 'divisions' => 'branchID' );
	...
	public function save() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('jobTitleID', 'positionCode', 'companyID', 'branchID', 'deptID', 'divisionID' );
		...
		if ($this->roles ['create']) {
			...
			if ($this->records->save ()) {
				$this->records->fields = array ();
				$id = $this->records->where ['jobPositionID'] = $this->db->insert_id ();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('jobTitleID', 'positionCode', 'companyID', 'branchID', 'deptID', 'divisionID', 'status' );
	...
	public function view($id) {
		...
		if ($this->roles ['view']) {
			...
			$this->db->select ( 'job_titles.code' );
			...
			$this->db->join ( 'job_titles', $this->table . '.jobTitleID=job_titles.jobTitleID', 'left' );
	...
	public function show() {
		...
		$condition_fields = array (array ('variable' => 'positionCode', 'field' => $this->table . '.positionCode', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'jobTitleID', 'field' => $this->table . '.jobTitleID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'companyID', 'field' => $this->table . '.companyID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'branchID', 'field' => $this->table . '.branchID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'deptID', 'field' => $this->table . '.deptID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'divisionID', 'field' => $this->table . '.divisionID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('positionCode' => 'asc' );
	...
	public function printlist() {
		...
		// variable:field:default_value:operator
		// note: dont include the special query field filter
		$condition_fields = array (array ('variable' => 'positionCode', 'field' => $this->table . '.companyID', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'jobTitleID', 'field' => $this->table . '.jobTitleID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'companyID', 'field' => $this->table . '.companyID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'branchID', 'field' => $this->table . '.branchID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'deptID', 'field' => $this->table . '.deptID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'divisionID', 'field' => $this->table . '.divisionID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->divisionName . "</Data></Cell>";
				if ($row->status == 1) {
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Vacant</Data></Cell>";
				} else if ($row->status == 2) {
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Occupied</Data></Cell>";
