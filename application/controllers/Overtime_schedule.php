...
class Overtime_schedule extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['controller_page'] = $this->controller_page = site_url('overtime_schedule'); // defines contoller link
		$this->table = 'overtime_schedules'; // defines the default table
		$this->pfield = $this->data['pfield'] = 'overtimeScheduleID'; // defines primary key
		$this->logfield = 'overtimeScheduleID';
		$this->module_path = 'modules/'.strtolower(str_replace(" ", "_", $this->module)) . '/overtime_schedule'; // defines module path
	...
	private function submenu()
	{
		...
		$this->data['current_module'] 		= $this->modules[$this->module]['sub']['Overtime Schedule']; // defines the current sub module
	...
	private function _in_used($id = 0) {
		$tables = array ('branches' => 'companyID', 'departments' => 'companyID', 'divisions' => 'companyID' );
		...
		if ($this->roles['create']){
			if(!empty($_SESSION['employees'])){
				...
				foreach($_SESSION['employees'] as $item){					
					...
					$this->records->fields['empID'] 		= $item['empID'];
					$this->records->fields['employmentID'] 	= $item['employmentID'];
					$this->records->fields['dateFiled'] 	= date('Y-m-d');
					$this->records->fields['date'] 			= date('Y-m-d', strtotime($item['startDate']));
					$this->records->fields['startTime'] 	= date('Y-m-d H:i:00', strtotime($item['startDate']));
					$this->records->fields['endTime'] 		= date('Y-m-d H:i:00', strtotime($item['endDate']));
					$this->records->fields['remarks'] 		= $item['remarks'];
					
					if ($this->records->save()) {
						// record logs
						$logs = "Record - ".$item['employment'];
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('empID', 'employmentID', 'remarks');
		...
		if ($this->roles ['edit']) {
			...
			$this->records->fields['startTime']	= date('Y-m-d H:i:00', strtotime($this->input->post('startDate')));
            $this->records->fields['endTime']  	= date('Y-m-d H:i:00', strtotime($this->input->post('endDate')));
	...
	public function approve_record($id=0) {
		// load submenu
		$this->submenu ();
		$data = $this->data;
		...
		if ($this->roles ['edit']) {
			...
			if ($this->records->update ()) {
				...
				$data ["class"] = "success";
				$data ["msg"] = $this->data ['current_module'] ['module_label'] . " successfully updated.";
				$data ["urlredirect"] = $this->controller_page . "/view/" . trim ($this->encrypter->encode($id));
				$this->load->view ( "header", $data );
				$this->load->view ( "message" );
				$this->load->view ( "footer" );
			} else {
	...
	public function disapprove_record($id=0) {
		// load submenu
		$this->submenu ();
		$data = $this->data;
	...
	public function view($id) {
		...
		if ($this->roles ['view']) {
			...
			$this->db->where('overtimeScheduleID',$id);
	...
	public function show() {
        ...
        $condition_fields = array(
			...
			array('variable'=>'startTime', 'field'=>$this->table.'.startTime', 'default_value'=>'', 'operator'=>'where'),
			array('variable'=>'endTime', 'field'=>$this->table.'.endTime', 'default_value'=>'', 'operator'=>'where'),
	...
	public function printlist() {
		...
		$data ['title'] = "Overtime Schedule List";
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				$hour = (strtotime($row->endTime) - strtotime($row->startTime)) / 60 / 60;
				 
				$data .= "<Row>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $ctr . ".</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->dateFiled. "</Data></Cell>";
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . date('m/d/Y h:i A', strtotime($row->startTime)) . "</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . date('m/d/Y h:i A', strtotime($row->endTime)) . "</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $hour . "</Data></Cell>";
				...
				if ($row->status == 1) {
					...
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Approved</Data></Cell>";
				} else if ($row->status == 0){
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Cancelled</Data></Cell>";
				} else if ($row->status == -1){
					$data .= "<Cell ss:StyleID='s24B'><Data ss:Type='String'>Disapproved</Data></Cell>";
				}
				$data .= "</Row>";
	...
	public function display_session_items($display_area='')
	{
		...
		$headers = array('Employee'=>'left','Employment'=>'left',' Start Time'=>'left', 'End Time'=>'left','Hours'=>'center','Remarks'=>'left');
		$display = array(
				array('align'=>'left','fields'=>array('employee'=>'')),
				array('align'=>'left','fields'=>array('employment'=>'')),
				array('align'=>'left','fields'=>array('startDate'=>'')),
				array('align'=>'left','fields'=>array('endDate'=>'')),
				array('align'=>'center','fields'=>array('hours'=>''),'display_total'=>true),
