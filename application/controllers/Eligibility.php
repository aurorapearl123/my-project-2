...
class Eligibility extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // set variables
        $this->data['current_module'] = $this->module = 'Service Eligibility';
        $this->module_label = 'Service Eligibilities';
        $this->table        = 'employee_eligibilities';
        $this->module_path  = 'modules/Employee/Eligibility';
        $this->module_path_submenu  = 'modules/Employee/submenu';
        $this->pfield = 'eligibilityID';
        $this->logfield = 'eligibility';
        $this->data['controller_page'] = $this->controller_page = site_url('eligibility');
    ...
    public function create()
    {
        ...
        if ($this->roles['create']) {
            $data['required_fields'] = array('empID'=>'Employee', 'eligibility'=>'Eligibility');
    ...
    public function save()
    {
        ...
        if ($this->roles['create']) {
            ...
            $table_fields = array('empID', 'eligibility', 'rating', 'examPlace','licenseNo', 'remarks');
            
            foreach($table_fields as $fld) {
                $this->record->fields[$fld] = trim($this->input->post($fld));
            }                                   
            
            $this->record->fields['examDate']       = ($this->input->post('examDate')!="") ? date('Y-m-d', strtotime(trim($this->input->post('examDate')))) : "0000-00-00";
            $this->record->fields['dateLicense']    = ($this->input->post('dateLicense')!="") ? date('Y-m-d', strtotime(trim($this->input->post('dateLicense')))) : "0000-00-00";
            $this->record->fields['dateExpired']    = ($this->input->post('dateExpired')!="") ? date('Y-m-d', strtotime(trim($this->input->post('dateExpired')))) : "0000-00-00";
            
            $this->record->fields['status']     = 1;
            if ($this->record->fields['dateExpired']!="0000-00-00") {
                if (strtotime($this->record->fields['dateExpired']) < time()) {
                    $this->record->fields['status'] = -1;
                }
            }
            if ($this->record->save()) {
                ...
                $this->record->where['eligibility']= trim($this->input->post('eligibility'));
    ...
    public function get_eligibilities()
    {
        $this->db->order_by('eligibility','asc');
        $records = $this->db->get('eligibilities');
        echo $this->frameworkhelper->get_json_data($records, 'eligibilityID', 'eligibility');
    }
