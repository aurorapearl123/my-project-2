...
class Section extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data ['controller_page'] = $this->controller_page = site_url ( 'section' ); // defines contoller link
		$this->table = 'divisions'; // defines the default table
		$this->pfield = $this->data ['pfield'] = 'divisionID'; // defines primary key
		$this->logfield = 'divisionCode';
		
		$this->module_path = 'modules/' . strtolower ( str_replace ( " ", "_", $this->module ) ) . '/section'; // defines module path
	...
	private function submenu() {
		...
		$this->data ['current_module'] = $this->modules [$this->module] ['sub'] ['Section']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	private function _in_used($id = 0) {
		$tables = array ('employments' => 'divisionID' );
	...
	public function save() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('companyID', 'branchID', 'deptID', 'divisionCode', 'divisionName', 'divisionAbbr', 'divisionHeadID', 'divisionHeadTitle', 'divisionContact', 'divisionEmail' );
		// check role
		if ($this->roles ['create']) {
			...
			if ($this->records->save ()) {
				$this->records->fields = array ();
				$id = $this->records->where ['divisionID'] = $this->db->insert_id ();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('companyID', 'branchID', 'deptID', 'divisionCode', 'divisionName', 'divisionAbbr', 'divisionHeadID', 'divisionHeadTitle', 'divisionContact', 'divisionEmail', 'status' );
	...
	public function show() {
		...
		$condition_fields = array (array ('variable' => 'divisionCode', 'field' => $this->table . '.divisionCode', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'divisionName', 'field' => $this->table . '.divisionName', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'deptID', 'field' => $this->table . '.deptID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'branchID', 'field' => $this->table . '.branchID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'companyID', 'field' => $this->table . '.companyID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'divisionEmail', 'field' => $this->table . '.divisionEmail', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'divisionAddress', 'field' => $this->table . '.divisionAddress', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'divisionHeadID', 'field' => $this->table . '.divisionHeadID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->divisionEmail . "</Data></Cell>";
