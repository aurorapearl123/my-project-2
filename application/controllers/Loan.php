...
class Loan extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->module_path  = 'modules/salary_and_wages/loan';           
        $this->data['controller_page']  = $this->controller_page = site_url('loan');
    ...
    private function submenu()
    {
        ...
        $this->data['current_module']   = $this->modules[$this->module]['sub']['Loan'];      // defines the current sub module
        // check roles
        $this->check_roles();
    ...
    public function save()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('employmentID','amount','isPayment');
        ...
        if ($this->roles['create']) {
            ...
            if (!empty($employment)) {
                $this->db->set('loanTypeID', trim($this->input->post('loanTypeID')));
                $this->db->set('empID', $employment->empID);
                $this->db->set('principal', trim($this->input->post('principal')));
                $this->db->set('payment', trim($this->input->post('payment')));
                $this->db->set('balance', trim($this->input->post('balance')));
                $this->db->set('dateFiled', date('Y-m-d', strtotime(trim($this->input->post('dateFiled')))));
    ...
    public function edit($id)
    {
        ...
        if ($this->roles['edit']) {
            ...
            $this->records->fields[] = 'loans.loanTypeID';
            $this->records->fields[] = 'loans.principal';
            $this->records->fields[] = 'loans.payment';
            $this->records->fields[] = 'loans.balance';
            $this->records->fields[] = 'loans.dateFiled';
            $this->records->fields[] = 'loans.startDate';
            $this->records->fields[] = 'loans.endDate';
            $this->records->joins[]  = array('loans',$this->table.'.payID=loans.payID','left');
            // set where
    ...
    public function update()
    {
        ...
        if ($this->roles['edit']) {
            ...
            if ($this->records->update()) {
                ...
                $this->db->where('detailID', $this->encrypter->decode($this->input->post($this->pfield)));
                $loanDetail = $this->db->get('loan_details', 1)->row();
                
                $this->records->table = 'loans';
                $this->records->fields = array();
                $table_fields = array('principal','payment','balance','remarks','status');
                
                foreach($table_fields as $fld) {
                    $this->records->fields[$fld] = trim($this->input->post($fld));
                }
                
                $this->records->fields['dateFiled'] = date('Y-m-d', strtotime(trim($this->input->post('dateFiled'))));
                $this->records->fields['startDate'] = date('Y-m-d', strtotime(trim($this->input->post('startDate'))));
                $this->records->fields['endDate']   = date('Y-m-d', strtotime(trim($this->input->post('endDate'))));
                
                $this->records->pfield   = 'payID';
                $this->records->pk       = $loanDetail->payID;
    ...
    public function view($id)
    {
        ...
        if ($this->roles['view']) {
            ...
            $this->db->select('loans.startDate');
            $this->db->select('loans.endDate');
    ...
    public function show()
    {
        ...
        $condition_fields = array(
			...
			array('variable'=>'loanTypeID', 'field'=>'loans.loanTypeID', 'default_value'=>'', 'operator'=>'where'),
    ...
    function exportlist()
    {
        ...
        $data = "<?xml version='1.0'?>
    		...
    		<Column ss:Index='13' ss:AutoFitWidth=\"1\" ss:Width='150.00'/>
    		<Column ss:Index='14' ss:AutoFitWidth=\"1\" ss:Width='80.00'/>
    		    ";
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='Number'>".$row->principal."</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='Number'>".$row->payment."</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='Number'>".$row->balance."</Data></Cell>";
    ...
    {
        $tables = array('payslip_deductions'=>'payID');
