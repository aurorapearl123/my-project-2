...
class Company extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data ['controller_page'] = $this->controller_page = site_url ( 'company' ); // defines contoller link
	...
	private function submenu() {
		...
		$this->data ['current_module'] = $this->modules [$this->module] ['sub'] ['Company']; // defines the current sub module
		// check roles
		$this->check_roles ();
	...
	public function save() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('companyCode', 'companyName', 'companyAbbr', 'companyContact', 'companyEmail', 'companyAddress', 'companyHeadID', 'companyHeadTitle' );
		...
		if ($this->roles ['create']) {
			...
			if ($this->records->save ()) {
				$this->records->fields = array ();
				$id = $this->records->where ['companyID'] = $this->db->insert_id ();
	...
	public function update() {
		...
		$this->submenu ();
		$data = $this->data;
		$table_fields = array ('companyCode', 'companyName', 'companyAbbr', 'companyContact', 'companyEmail', 'companyAddress', 'companyHeadID', 'companyHeadTitle', 'status' );
	...
	public function view($id) {
		...
		if ($this->roles ['view']) {
			...
			$this->db->join ( 'employees', $this->table . '.companyHeadID=employees.empID', 'left' );
	...
	public function show() {
		...
		$condition_fields = array (array ('variable' => 'companyCode', 'field' => $this->table . '.companyCode', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'companyName', 'field' => $this->table . '.companyName', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'companyEmail', 'field' => $this->table . '.companyEmail', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'companyAddress', 'field' => $this->table . '.companyAddress', 'default_value' => '', 'operator' => 'like_both' ), array ('variable' => 'companyHeadID', 'field' => 'employees.empID', 'default_value' => '', 'operator' => 'where' ), array ('variable' => 'status', 'field' => $this->table . '.status', 'default_value' => '', 'operator' => 'where' ) );
		
		// sorting fields
		$sorting_fields = array ('companyCode' => 'asc' );
	...
	function exportlist() {
		...
		if (count ( $records )) {
			...
			foreach ( $records as $row ) {
				...
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->companyEmail . "</Data></Cell>";
				$data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>" . $row->companyAddress . "</Data></Cell>";
	...
	public function check_duplicate() {
		$this->db->where ( 'companyCode', trim ( $this->input->post ( 'companyCode' ) ) );
