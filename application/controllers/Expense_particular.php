...
class Expense_particular extends CI_Controller
{
    //Default Variables
    var $menu;
    var $roles;
    ...
    public function __construct()
    {
        ...
        $this->data['controller_page']  = $this->controller_page = site_url('expense_particular');// defines contoller link
        $this->table        = 'expense_particulars';                                                  // defines the default table
        $this->pfield = $this->data['pfield'] = 'particularID';                                                 // defines primary key
        // $this->logfield     = 'deptCode';
        $this->module_path  = 'modules/'.strtolower(str_replace(" ","_",$this->module)).'/expense_particular';// defines module path / folder name
    ...
    private function submenu()
    {
        ...
        $this->data['current_module']        = $this->modules[$this->module]['sub']['Expense Particulars'];      // defines the current sub module
        // check roles
        $this->check_roles();
    ...
    public function save()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('particular','description');
        ...
        if ($this->roles['create']) {
            ...
            if ($this->record->save()) {
                $this->record->fields = array();
                $particularID = $this->record->where['particularID'] = $this->db->insert_id();
                ...
                $data["urlredirect"] = $this->controller_page."/view/".$this->encrypter->encode($particularID);
                $this->load->view("header",$data);
                $this->load->view("message");
                $this->load->view("footer");
    ...
    public function show()
    {
        ...
        $condition_fields = array(
            array('variable'=>'particular', 'field'=>$this->table.'.particular', 'default_value'=>'', 'operator'=>'like_both'),
        ...
        // sorting fields
        $sorting_fields = array('particular'=>'asc');
    ...
    public function printlist()
    {
        ...
        $data['title'] = "Expense Particular List";
    ...
    function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->particular."</Data></Cell>";
    ...
    {
        $this->db->where('particularID', trim($this->input->post('particularID')));      
