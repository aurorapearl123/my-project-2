...
class Service_type extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->data['controller_page']  = $this->controller_page = site_url('service_type');// defines contoller link
        $this->table        = 'service_types';                                                  // defines the default table
        $this->pfield = $this->data['pfield'] = 'serviceID';                                                 // defines primary key
        // $this->logfield     = 'deptCode';
        $this->module_path  = 'modules/'.strtolower(str_replace(" ","_",$this->module)).'/service_type';             // defines module path
    ...
    private function submenu()
    {
        ...
        $this->data['current_module']        = $this->modules[$this->module]['sub']['Service Types'];      // defines the current sub module
        // check roles
        $this->check_roles();
    ...
    public function save()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('serviceType','regRate','discountedRate');
        ...
        if ($this->roles['create']) {
            ...
            if ($this->record->save()) {
                $this->record->fields = array();
                $serviceID = $this->record->where['serviceID'] = $this->db->insert_id();
                ...
                $data["urlredirect"] = $this->controller_page."/view/".$this->encrypter->encode($serviceID);
                $this->load->view("header",$data);
                $this->load->view("message");
                $this->load->view("footer");
    ...
    public function update()
    {
        ...
        $this->submenu();
        $data = $this->data;
        $table_fields = array('serviceType','regRate','discountedRate','status');
    ...
    public function show()
    {
        ...
        $condition_fields = array(
            array('variable'=>'serviceType', 'field'=>$this->table.'.serviceType', 'default_value'=>'', 'operator'=>'like_both'),
            array('variable'=>'regRate', 'field'=>$this->table.'.regRate', 'default_value'=>'', 'operator'=>'like_both'),
            array('variable'=>'discountedRate', 'field'=>$this->table.'.discountedRate', 'default_value'=>'', 'operator'=>'like_both'),
            array('variable'=>'status', 'field'=>$this->table.'.status', 'default_value'=>'', 'operator'=>'where'),
        );
        
        // sorting fields
        $sorting_fields = array('serviceType'=>'asc');
    ...
    function exportlist()
    {
        ...
        if (count($records)) {
            ...
            foreach ($records as $row) {
                ...
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->regRate."</Data></Cell>";
                $data .= "<Cell ss:StyleID='s27'><Data ss:Type='String'>".$row->discountedRate."</Data></Cell>";
                if  ($row->status == 1) {
    ...
    {
        $this->db->where('serviceID', trim($this->input->post('serviceID')));      
