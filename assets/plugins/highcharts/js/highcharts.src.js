...
/**
 * Remove last occurence of an order from an array
 * @param {Array} arr
 * @param {Mixed} item
 */
...
(function ($) {
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
		...
		/**
		 * Return the index of an order in an array, or -1 if not found
		 */
		inArray: $.inArray,
...
SVGRenderer.prototype = {
	...
	/**
	 * Add a label, a text order that can hold a colored or gradient background
...
Axis.prototype = {
	...
	render: function () {
		...
		if (hasData || isLinked) {
			...
			// Major ticks. Pull out the first order and render it last so that
...
Legend.prototype = {
	...
	/**
	 * Set the colors for the legend order
	...
	/**
	 * Render a single specific legend order
	...
	renderItem: function (item) {
		...
		if (!li) { // generate it once, later move it
			...
			item.legendGroup = renderer.g('legend-order')
				.attr({ zIndex: 1 })
			...
			// Generate the list order text and add it to the group
			...
			// Set the events on the order group, or in case of useHTML, the order itself (#1249)
		...
		// if the order exceeds the width, start a new line
		...
		// If the order exceeds the height, start a new column
	...
	render: function () {
		...
		each(chart.series, function (serie) {
			...
			// use points or series for the legend order depending on legendType
